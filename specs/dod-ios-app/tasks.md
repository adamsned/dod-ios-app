...
### T-550 — Settings page skeleton: gear icon + metric toggle + About Me placeholder + version footer (US-32, CL-56)
- **Scope:** Ship the v1 skeleton of the Settings page graduated from round-4 backlog. Three deliverables in one PR: (1) gear icon on the trailing edge of the Recipes (Feed) tab's nav bar that pushes a new `SettingsView` via `NavigationLink`; (2) `SettingsView` rendered as a `.listStyle(.insetGrouped)` list matching the post-T-560 Categories tab treatment (`.scrollContentBackground(.hidden) + .background(DODColor.surface)`) with three rows — "Use metric units" Toggle bound to a UserDefaults-backed flag, "About Dutch Oven Daddy" NavigationLink pushing a placeholder destination, and a version footer reading `"v\(version) (\(build))"` from `Bundle.main.infoDictionary`; (3) the minimum L1 + L4 test coverage to lock the toggle's UserDefaults round-trip and the rendered light + dark snapshots. The two sub-features the row UI implies — actual ingredient conversion behind the metric toggle, and live WP REST fetch behind the About Me NavigationLink — are explicit follow-up tasks (T-551 + T-552 below) so the v1 PR stays bounded to "ship the entry point." CL-56 walks through the skeleton-first split and the rationale for bundling SettingsView inside `DODFeatureFeed` instead of creating a new Swift package (avoids project.yml churn for a single view; can extract later if Settings grows past one screen).
- **Files:**
  - **Spec/clarifications/tasks (commit 1):** `specs/dod-ios-app/spec.md` (US-32 added after US-31 with 5 ACs), `specs/dod-ios-app/clarifications.md` (CL-56 appended), `specs/dod-ios-app/tasks.md` (this entry + T-551 + T-552 follow-ups under Phase 10), `specs/dod-ios-app/backlog.md` (Settings entry graduated).
  - **Source (commit 2):** `Packages/DODFeatureFeed/Sources/DODFeatureFeed/SettingsView.swift` (new), `Packages/DODFeatureFeed/Sources/DODFeatureFeed/SettingsViewModel.swift` (new — UserDefaults binding for the metric toggle), `Packages/DODFeatureFeed/Sources/DODFeatureFeed/FeedView.swift` (adds the `.toolbar` gear-icon `NavigationLink` to SettingsView on the trailing edge).
  - **Tests (commit 2):** `Packages/DODFeatureFeed/Tests/DODFeatureFeedTests/SettingsViewModelTests.swift` (new — L1 unit test for the toggle's UserDefaults persistence), `Packages/DODFeatureFeed/Tests/DODFeatureFeedTests/SettingsViewSnapshotTests.swift` (new — L4 snapshot tests for light + dark).
  - **Out of bounds:** no `project.yml` edit, no new Swift package, no `DODDomain` / `DODNetworking` / `DODPersistence` edits, no asset-catalog edits, no `DODColor` token additions, no recipe-detail code (T-551 follow-up), no WP REST client edits (T-552 follow-up), no analytics event additions, no deep-link grammar edits, no widget surface edits, no snapshot wire-format edits.
- **AC:** Implements US-32 AC-32.1 (gear icon trailing edge of Recipes nav bar pushes SettingsView), AC-32.2 (.insetGrouped list matching Categories), AC-32.3 (three rows: Toggle, NavigationLink, version footer), AC-32.4 (toggle persists to UserDefaults), AC-32.5 (US-4 not regressed). Pins AC-1.1..AC-1.7 (Feed contract unchanged; only a `.toolbar` modifier added).
- **Deps:** T-560 (Categories brown revert) — depends on T-560's `surface` treatment being on `main` so SettingsView's `.background(DODColor.surface)` lands on the same brand palette. Parallel with T-560 and T-570 (dark foreground refinement); source-side collisions zero (different files). Spec-file collisions on `clarifications.md` / `spec.md` / `tasks.md` / `backlog.md` are expected at merge time and the rebaser picks the next free CL slot (T-560 reserved CL-54; T-550 takes CL-56).
- **Est:** 2h (one new view + one new view-model + one FeedView toolbar add + two test files + spec/CL/tasks/backlog commit + manual sim verification + PR body assembly).
- **||:** P10-settings (new cluster, owns `Packages/DODFeatureFeed/**` Settings additions for round 6; sequences forward into T-551 + T-552).

### T-551 — Metric units conversion: wire UserDefaults flag into recipe-detail + Cook Mode ingredient rendering (US-32, follow-up to T-550)
- **Scope:** Follow-up to T-550. Plumb the `dod.settings.useMetricUnits` UserDefaults flag into the ingredient-rendering path so toggling the Settings switch actually re-renders ingredient quantities (cups → grams, oz → g, lbs → kg, °F → °C). Extends `FractionRenderer` in `DODSupport` (CL-52's existing scaling utility) with a `metricEquivalent(...)` transform that runs after the per-render scaling step. Call sites: `RecipeDetailView.ingredientsSection` (passes the flag through `displayText` shim) and `CookModeView.ingredientsDrawer` (passes the flag through `ingredientScaleFactor`'s sibling param). The transform table covers the common US-recipe units (cup, tablespoon, teaspoon, oz, lb, °F) and pass-throughs anything it doesn't recognize. L1 golden table in `FractionRendererTests` pins the conversion math. No new analytics, no new tokens, no schema bump.
- **Deps:** T-550 (must ship first — provides the UserDefaults flag).
- **Est:** 3h.
- **||:** P10-settings.

### T-552 — About Me WP REST fetch: replace placeholder with live `/wp/v2/pages?slug=about-me` content + offline cache (US-32, follow-up to T-550)
- **Scope:** Follow-up to T-550. Replace the "Coming soon — fetched from /about-me/" placeholder destination view with a live WP REST fetch from `https://www.dutchovendaddy.com/wp-json/wp/v2/pages?slug=about-me`. Mirrors the existing CL-1 hybrid pattern: fetch on appear, cache the rendered content (HTML-stripped to readable Swift text) so the screen works offline, surface the offline banner if the network fails and there's no cached copy. Adds `WPRestClient.pages(slug:)` method (parallels the existing `posts()` / `categories()` / `search()` shape). Optionally adds a `CachedAboutMe` schema entry, or stores the cached blob in UserDefaults if we want to avoid the schema bump. L1 coverage for the WP REST decode + HTML strip; L4 snapshot for the rendered About Me view in light + dark.
- **Deps:** T-550 (must ship first — provides the NavigationLink entry point).
- **Est:** 3h.
- **||:** P10-settings.
...
### T-570 — Dark-mode SurfaceElevated brightness lift (US-30 amended, CL-55)
- **Scope:** Single asset-catalog hex swap in `Packages/DODDesignSystem/Sources/DODDesignSystem/Resources/Colors.xcassets/SurfaceElevated.colorset/Contents.json` — the Dark Appearance entry moves from `#281F19` (R `0x28` / G `0x1F` / B `0x19`) to `#5A3520` (R `0x5A` / G `0x35` / B `0x20`), a ~8% brightness lift within the same warm-brown hue family. CL-55 captures the rationale: the original T-520 / CL-51 dark hex was *darker* than the new `#42210B` Surface, so elevated surfaces (cards, sheets, chips) read as recessed below the page background rather than raised above it — the inverse of Apple's dark-mode "card rises above background" depth pattern. The new value restores the elevated-above-base relationship. Light Appearance stays `#FFFFFF` (unchanged), `Surface.colorset` is untouched, and no other tokens (`Label`, `LabelSecondary`, `Charcoal`, `Accent`, `CastIronBrown`, `BurntOrange`, `WarmGold`, `DarkEarth`, `Cream`, `CreamSubtle`) are touched. WCAG AA contrast against the unchanged dark `Label` (`#E6DECF` cream) clears 4.5:1 by a wide margin — the lighter SurfaceElevated actually *improves* foreground contrast relative to the prior `#281F19`.
- **Files:**
  - **Spec/clarifications/tasks (commit 1):** `specs/dod-ios-app/spec.md` (AC-30.2 amended in place to reference the new `#5A3520` Dark Appearance hex), `specs/dod-ios-app/clarifications.md` (CL-55 appended after CL-54), `specs/dod-ios-app/tasks.md` (this entry under Phase 10).
  - **Source (commit 2):** `Packages/DODDesignSystem/Sources/DODDesignSystem/Resources/Colors.xcassets/SurfaceElevated.colorset/Contents.json` — three byte changes (R/G/B hex values in the Dark Appearance entry).
  - **Out of bounds:** every snapshot baseline (intentionally deferred to T-571 — see "Snapshot baseline deferral" below), the `Surface.colorset` (untouched), every other `*.colorset` directory, `Colors.swift`, every Swift source file in the repo, `bin/format.sh`, `swiftlint.yml`, `xcconfig` files, `project.yml`, the widget snapshot wire format, deep-link grammar, analytics events. No new token, no rename, no call-site rewrite — every call site that already reads `DODColor.surfaceElevated` automatically picks up the new dark hex via the asset catalog.
- **Snapshot baseline deferral (T-571 follow-up):** the dark-mode `SurfaceElevated` retint drifts every `__Snapshots__/**/*.png` baseline rendered in dark mode (most of `DODDesignSystemTests`, the feature snapshot suites under `DODFeatureCategoriesTests` / `DODFeatureRecipeDetailTests` / `DODFeatureSearchTests`, and the widget appearance suites under `WidgetCardTintedAppearanceSnapshotTests` + `SavedWidgetSnapshotTests`). The existing tests use `record: .missing` so untouched PNGs stay byte-identical; T-570 intentionally does NOT delete and re-record any baseline here to keep the PR diff a single human-reviewable Contents.json edit. The re-record is filed as T-571 (Phase 10) for a future CI/iOS-sim job that can land the regenerated PNGs in isolation. AC-30.5's "every L4 snapshot baseline... is re-recorded" remains the long-term contract; T-571 is the scheduled satisfaction.
- **AC:** Satisfies AC-30.2 as amended by CL-55 (Dark Appearance entry uses `#5A3520` rather than `#281F19`). Pins AC-30.1 (Surface hex values unchanged), AC-30.3 (no edits outside the two surface colorsets), AC-30.4 (WCAG AA contrast preserved). Defers AC-30.5 (snapshot re-record) to T-571 with the documented `record: .missing` carry-over.
- **Deps:** T-520 (US-30 / CL-51) — depends on T-520's `Surface.colorset` re-tint already being on `main` so the `#42210B` Surface background exists for the new `#5A3520` SurfaceElevated to lift above. T-520 is merged. Parallel with T-560 (Categories surface revert) and T-550 (Settings page) — different files; source-side collisions zero. Spec-file collisions on `clarifications.md` / `spec.md` / `tasks.md` are expected at merge time and the rebaser picks the next free CL slot (T-560 reserved CL-54; T-570 reserves CL-55).
- **Est:** 20m (single hex swap + spec/CL/tasks commit + manual sim verification + PR body).
- **||:** P10-dark-foreground (independent of every other Phase 10 task source-wise).

### T-571 — Re-record dark-mode snapshot baselines after T-570 (US-30 follow-up)
- **Scope:** Follow-up to T-570 that deletes and re-records every snapshot baseline whose rendered surface includes `DODColor.surfaceElevated` in dark mode. Touches no production source — only the regenerated `__Snapshots__/**/*.png` artifacts under `DODDesignSystemTests`, the feature snapshot suites, and the widget appearance suites. Runs on an iOS-sim CI runner where the iPhone simulator can render the new `#5A3520` hex against the existing test fixtures and emit fresh PNGs. Tests use `record: .missing` so the re-record happens by deleting each existing PNG, re-running the test suite, and committing the regenerated artifacts. Reviewer spot-checks a representative cross-section (≥5 PNGs covering DesignSystem components, a feature view, and a widget face) to confirm the new dark SurfaceElevated reads as lifted from the `#42210B` Surface.
- **Files:** `Packages/DODDesignSystem/Tests/DODDesignSystemTests/__Snapshots__/**/*.png` (dark-mode subset), the feature snapshot suites' `__Snapshots__/**/*.png` (dark-mode subset), and the widget appearance suites' `__Snapshots__/**/*.png` (dark-mode subset). No `.swift` file edits, no `xcassets` edits, no spec/CL/tasks edits.
- **AC:** Satisfies AC-30.5 (every L4 snapshot baseline that renders `Surface` or `SurfaceElevated` in dark mode is re-recorded post-T-570).
- **Deps:** T-570 (the `#5A3520` hex must be on `main` so the regenerated PNGs reflect the new value).
- **Est:** 1h (delete dark-mode PNGs, re-run snapshot suite on iOS-sim runner, commit regenerated artifacts, spot-check sample).
- **||:** P10-dark-foreground (same cluster as T-570; sequential after T-570).

---

## Phase 11 — Test pyramid L5 E2E (2026-05-26)

Adds a fifth test layer (L5 end-to-end user journeys) atop the existing L1–L4. The five-layer pyramid is the **first** place we deliberately introduce selective gating — L5 runs on the `e2e` PR label, on push to `main`, on `workflow_dispatch`, and nightly. Every other layer continues to run on every PR per constitution §6. See [`test-pyramid-audit.md`](test-pyramid-audit.md) for the layer matrix and the gap analysis that motivated this cluster.

### T-600 — Test pyramid audit (CL-58 framing)
- **Scope:** Audit-style task framed by CL-58. Inventory each existing layer (L1/L2/L3/L4) with what it tests, where it lives, when it runs, who's expected to add tests for each PR type. Flag the gap: today's L3 "smoke" mixes lightweight reachability assertions (2s each) with full-journey walks (15–45s each, hitting the live blog). The constitution-§6 promise "all four layers run in CI; red blocks merge" applies cleanly to L1/L2/L4 but creaks at L3 because the two jobs share a scheme. Audit doc proposes adding L5 (E2E) as a fifth layer with selective gating (`e2e` PR label + push-to-main + nightly + workflow_dispatch). Per CL-58, a clean audit that lands the document and the matrix is the deliverable here; the implementation tasks (T-602..T-605) are siblings, not nested.
- **Files:** `specs/dod-ios-app/test-pyramid-audit.md` (new — sibling of `appearance-audit.md` / `accessibility-audit.md` / `widget-appearance-audit.md`; ~120 lines, table-heavy), `specs/dod-ios-app/tasks.md` (this Phase 11 cluster).
- **AC:** Locks the L5 layer name + scheme (`DODAppE2ETests`), the four trigger surfaces, the test-data strategy (launch-arg stub now, host-side fake follow-up T-610), the selective-gating predicate, and the seed-five user-journey list. Pins constitution §6 — does **not** restructure L1–L4. Implements AC-T5 (added by Phase 11 spec amendment in T-601).
- **Deps:** —
- **Est:** 1.5h
- **||:** P11-test-pyramid

### T-601 — L5 spec amendment + CL-58
- **Scope:** Three artifacts. (1) `specs/constitution.md` §6 — append L5 to the layer table with: what L5 tests, what tool (XCUITest), what scheme (`DODAppE2ETests`), when it runs (label-gated PR + push + nightly + workflow_dispatch — **not** every PR), what kind of changes warrant the label. Add the explicit carve-out that L5 is the first layer the constitution allows selective gating for, and the rationale (CI quota + signal-to-noise) cross-references CL-58. (2) `specs/dod-ios-app/clarifications.md` — new **CL-58** entry capturing the decision rationale: why selective gating (CI quota + signal-to-noise; the existing 4-layer "every PR" pyramid stays unchanged), why XCUITest over Detox/Maestro/WebDriverAgent (no new dep family; existing L3 author skillset transfers), why the four trigger surfaces (label-gated PR for coverage on risky changes, push to main as the merged-state safety net, nightly as the env-drift detector, workflow_dispatch as the manual escape hatch), what we deliberately said NO to (no L5 visual-regression coverage — L4's job; no live-blog writes — read-only L2 contract still applies; no Android E2E layer — iOS-only repo). (3) `specs/dod-ios-app/spec.md` — append AC-T5 to the "Test pyramid" section covering the L5 layer.
- **Files:** `specs/constitution.md` (§6 amendment — additive L5 row + selective-gating paragraph), `specs/dod-ios-app/clarifications.md` (new CL-58 entry under a new "Phase 11 amendments" section header), `specs/dod-ios-app/spec.md` ("Test pyramid" block — append AC-T5).
- **AC:** AC-T5 (new — L5 layer exists, is XCUITest, lives at `DODAppE2ETests`, runs on label-gated PR + push + nightly + workflow_dispatch); pins AC-T1..AC-T4 (the existing four layers are unchanged).
- **Deps:** T-600 (audit doc is the spec-trace context for this amendment).
- **Est:** 1h
- **||:** P11-test-pyramid

### T-602 — E2E test target + scheme + wiring
- **Scope:** Stand up the new XCUITest target `DODAppE2ETests`. New scheme so the L3 smoke scheme (`DODAppUITests`) stays fast and runs every PR. The new target compiles `UITests/E2E/**` (new subdirectory, sibling of the existing `UITests/SmokeTests.swift` + `UITests/RecipeDetailRatingsSmokeTests.swift`). Test-data strategy: deterministic launch via `XCUIApplication.launchArguments += ["-DOD_E2E_MODE=1"]` + `XCUIApplication().launchEnvironment["DOD_E2E_MODE"] = "1"`; the host app reads the flag at boot via a new branch in `App/DODApp.swift` `applyTestLaunchOverrides()` that records the flag into a process-wide `DODEnvironment.isE2EMode` boolean. **Phase 1 (this task):** the boolean is recorded but does not yet swap dependencies — the seed journeys in T-603 drive against the existing production code paths (same as the existing L3 smoke job today). **Phase 2 (T-610 follow-up):** wire a `FakeAppDependencies` that returns canned fixtures so the suite runs hermetically. Both phases ship; the second phase is the host-side switch the audit doc flags. `project.yml` gains a new `DODAppE2ETests` `bundle.ui-testing` target with `TEST_TARGET_NAME: DODApp` and the existing `DODAppUITests` target gains an explicit `sources.excludes: ["E2E"]` so the two schemes don't double-compile. Regenerate with `xcodegen generate`. **No baseline screenshots / no `record: .missing` PNGs** — XCUITest is behavioral, not visual; L4's job is unchanged.
- **Files:** `project.yml` (new `DODAppE2ETests` target ~25 LOC + existing `DODAppUITests` `excludes` line); `App/DODApp.swift` (new branch in `applyTestLaunchOverrides()` reading `-DOD_E2E_MODE` / env `DOD_E2E_MODE=1`); new `UITests/E2E/` directory; new `UITests/E2E/E2ETestSupport.swift` (tiny ~30-LOC helper exposing a `XCUIApplication.launchForE2E()` extension that sets the launch arg + env + telemetry-suppress flag in one place so the five seed journeys share the boilerplate).
- **AC:** Implements AC-T5 (locked in T-601). New `DODAppE2ETests` scheme builds and `xcodebuild test -scheme DODAppE2ETests -only-testing:DODAppE2ETests` returns success with zero journeys defined yet (T-603 adds the methods). Existing `DODAppUITests` scheme still builds and runs identically — no regression in the L3 smoke contract.
- **Deps:** T-601 (the scheme name is locked by the spec amendment; pinning that first avoids a rename later).
- **Est:** 1.5h
- **||:** P11-test-pyramid

### T-603 — Initial E2E user journeys (seed five)
- **Scope:** Five concrete user journeys, each a distinct `func test_…()` method in `UITests/E2E/CoreUserJourneysE2ETests.swift`. Each method launches the app fresh via `XCUIApplication().launchForE2E()`, walks a real user task to completion, asserts one meaningful end-state condition, and runs in under 30s wall-clock on the iPhone 17 simulator. Seed five:
  1. **`test_firstLaunch_onboarding_then_tap_recipe_sees_detail`** — fresh-onboarding launch → "Get cooking" dismisses welcome sheet → feed loads → tap first recipe row → recipe detail visible (Ingredients header present).
  2. **`test_search_recipes_tap_result_sees_detail`** — open Search tab → type a query into the "Search recipes" text field → wait for results → tap first result → recipe detail visible.
  3. **`test_save_recipe_then_unsave_from_saved_tab`** — open recipe → tap "Save recipe" → switch to Saved tab → confirm row exists → tap into it → tap "Unsave recipe" → back to Saved tab → confirm "No saved recipes yet" empty state.
  4. **`test_cook_mode_walks_two_steps_then_exits`** — open recipe → tap "Cook Now" → see "Step 1 of M" → tap "Next" → see "Step 2 of M" → tap "Done" → back at recipe detail (Ingredients header visible).
  5. **`test_widget_deeplink_opens_recipe_detail`** — launch with `launchArguments += ["-DODOpenURL", "dod://recipe/<id>"]` where `<id>` is a known fixture recipe id (sourced at runtime from a default-launched probe so the test isn't pinned to a specific live-blog id) → app opens directly to recipe detail with the Ingredients header visible.
  Two of the seed five overlap with existing L3 smoke methods (`test_onboardingShowsOnFirstLaunchAndDismisses` and `test_cookModeOpensAndAdvances`). The L3 versions are not removed in this task — they continue to run on every PR as fast reachability checks. The L5 versions duplicate the walk at a higher fidelity (end-to-end vs reachability) and are tracked as a future de-dupe in T-612 once the L5 scheme has proven green over a week of normal merges. If any of the five requires production-target accessibility-identifier additions, cap at ~10 identifier additions total in this commit; don't refactor existing views.
- **Files:** new `UITests/E2E/CoreUserJourneysE2ETests.swift` (one file, ~250 LOC = five test methods + tiny helpers); accessibility-identifier additions to production views if reachable user-visible labels are insufficient (cap ~10 lines across `Packages/DODFeatureRecipeDetail/Sources/DODFeatureRecipeDetail/RecipeDetailView.swift` / `Packages/DODFeatureFeed/Sources/DODFeatureFeed/FeedView.swift` / `Packages/DODFeatureSearch/Sources/DODFeatureSearch/SearchView.swift` — only as needed). **Out of bounds:** removing the L3 duplicates (`test_onboardingShowsOnFirstLaunchAndDismisses`, `test_cookModeOpensAndAdvances`) — that's T-612's territory; new fake-dependencies infrastructure — T-610's territory; new visual-regression baselines — L4's territory, deliberately not extended for L5.
- **AC:** Implements AC-T5 (locked in T-601). All five seed journeys pass on the iPhone 17 simulator in under 30s each. `xcodebuild test -scheme DODAppE2ETests -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:DODAppE2ETests` returns success.
- **Deps:** T-602 (the target + scheme must exist before the journeys land in it).
- **Est:** 2.5h
- **||:** P11-test-pyramid

### T-604 — CI workflow for L5 (label gating + nightly)
- **Scope:** Add a new job `test-e2e` to `.github/workflows/ci.yml` that runs the L5 suite under the selective gating predicate. The job triggers on: (1) `pull_request` ONLY when the PR carries the `e2e` label, (2) `push` to `main`, (3) `workflow_dispatch` (manual), (4) a new top-level `schedule: '0 7 * * *'` (nightly 7am UTC — same wall clock as `nightly-live-api.yml` so both early-warning surfaces fire at the same time and don't compete for the morning runner pool). Reuses the existing `ios_sim` path-filter output so the job is also skipped on docs-only PRs even when the label is set (no point burning runner minutes on a docs PR that happens to be in the same branch as an unrelated `e2e`-labeled feature). Selects Xcode 16 like every other `ios_sim` job. Runs `xcodebuild test -scheme DODAppE2ETests -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:DODAppE2ETests test`. Uploads `*.xcresult` on failure as an artifact (same pattern as `test-ui-smoke`). Updates the `ci-required` aggregator: **skipped is success** for `test-e2e` (so PRs without the `e2e` label aren't blocked; only failures are blocking). The `on:` block at the top of the file extends with `schedule` + `workflow_dispatch` triggers — confirm the YAML is still valid via `python3 -c "import yaml; yaml.safe_load(open('.github/workflows/ci.yml'))"` after the edit.
- **Files:** `.github/workflows/ci.yml` (new `test-e2e` job ~30 LOC + new top-level `schedule` / `workflow_dispatch` triggers + `ci-required` aggregator update).
- **AC:** Implements AC-T5 (locked in T-601). The new `test-e2e` job triggers on the four documented surfaces and skips on every other PR. `gh pr view` shows the new check in the merge gate; "skipped" counts as success in the `ci-required` aggregator. Existing `test-ui-smoke` job is unchanged — L3 keeps running every PR.
- **Deps:** T-602, T-603 (the scheme + the journeys must exist before CI invokes them).
- **Est:** 1h
- **||:** P11-test-pyramid

### T-605 — CONTRIBUTING.md policy update
- **Scope:** Add a new section **"Does my PR need E2E?"** to `CONTRIBUTING.md` with a checklist style. Lists eight surfaces that warrant the `e2e` label (navigation/routing, persistence schema, composition root, widget code, Cook Mode state machine, comments/ratings submission, App Intents / Spotlight, onboarding) and five surfaces that don't (pure docs/spec, tests-only, lint/format/CI fixes, single-package refactor with no behavior change, bug fix proven by L1/L2 regression test). Also extend the existing "Local checks before pushing" block to include `xcodebuild test -scheme DODAppE2ETests …` as an **optional** pre-push command for PRs that will carry the `e2e` label. Cross-link the new section to the `test-pyramid-audit.md` audit doc (T-600) and to CL-58 in `clarifications.md` (T-601) so the rationale is one click away from the checklist.
- **Files:** `CONTRIBUTING.md` (new section + pre-push command addition; ~40 LOC net).
- **AC:** Implements AC-T5 (locked in T-601). PR authors have a one-page decision guide: read the checklist, decide whether to apply the `e2e` label, ship. No new tooling, no new git hook — the label is set in the PR UI per the existing labels convention.
- **Deps:** T-600 (cross-references the audit doc), T-601 (cross-references CL-58), T-604 (the CI surface the policy gates).
- **Est:** 0.75h
- **||:** P11-test-pyramid
...
- **Total tasks:** 73 (Phase 1–5) + 6 (Phase 6 consultant pass) + 5 (Phase 7 comments + ratings) + 6 (Phase 8 polish: T-310, T-320, T-321, T-322, T-323, T-330) + 5 (Phase 8 follow-ups surfaced by T-330: T-331, T-332, T-333, T-334, T-335) + 1 (Phase 9 categories modernization: T-340) + 17 (Phase 10: T-350, T-360, T-361, T-370, T-380, T-390, T-394, T-395, T-400, T-410, T-420, T-430, T-440, T-500, T-510, T-530, T-560) + 6 (Phase 11 test pyramid L5: T-600, T-601, T-602, T-603, T-604, T-605) = 119
- **Total estimate:** ~143 hours + ~17 hours (Phase 6) + ~19 hours (Phase 7) + ~13 hours (Phase 8) + ~9 hours (Phase 8 follow-ups) + ~2 hours (Phase 9) + ~27 hours (Phase 10) + ~8.25 hours (Phase 11) = ~238.25 hours
