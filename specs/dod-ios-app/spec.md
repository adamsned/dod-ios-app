...
- This story explicitly amends T-340 / CL-32's "lean into the system style, not invent" framing on one axis (the surface color is now a brand override instead of the system grouped background) and preserves it on every other axis (list style, row composition, search field, chevron, separators). CL-40 explains why the surface color is the right place to introduce the brand override and why the cell fill is the wrong place.
