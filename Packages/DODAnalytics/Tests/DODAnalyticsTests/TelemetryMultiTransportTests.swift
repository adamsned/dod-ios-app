...
/// Telemetry.shared is process-global; tests must run serially. Nested
/// under `TelemetrySharedStateGroup` so serialization applies across
/// every suite that touches the shared instance, not just this one.
extension TelemetrySharedStateGroup {
    @Suite("Telemetry multi-transport fan-out") struct TelemetryMultiTransportTests {
