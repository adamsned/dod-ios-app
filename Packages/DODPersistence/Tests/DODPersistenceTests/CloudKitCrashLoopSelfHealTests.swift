...
/// guard. These pin the pure logic the device fix relies on so CI validates
/// it without a device, an iCloud account, or the async CloudKit mirroring
/// path that can only be reproduced on a real device:
///
...
///
/// DUT-630 already covers the Simulator instance of the async-mirroring
/// trap (`cloudKitMirroringAvailable == false` there); these guards close
/// the remaining real-device instance, where the runtime is fine but the
/// *account* can't back CloudKit.
@Suite("CloudKit crash-loop self-heal (DUT-78)")
struct CloudKitCrashLoopSelfHealTests {
    ...
    /// Mirrors `CloudKitContainerSelectionTests.isolatedDefaults`.
    ...
    @Test func availableAccountOnOptInDoesNotForceFallback() throws {
        ...
        // `cloudKitDatabase: .none` regardless (an in-memory `.private` open
        // spins up NSCloudKitMirroringDelegate → PushKit and aborts a test
        // host with no CloudKit entitlement), so this exercises the branch
        // SELECTION without opening a real CloudKit container. The actual
        // synchronous `.private` open + its DOD-CRASH-1 catch are
        // device-verified, not unit-tested (see `CloudKitContainerSelectionTests`).
        let result = try RecipeStore.productionContainer(
            defaults: defaults,
    ...
    @Test func legacyEntryPointMatchesPreDUT78Behavior() throws {
        // The pre-existing `productionContainer(defaults:inMemory:cloudKitAvailable:)`
        // entry point (no probe, no tracker) is untouched: opt-out opens local
        // with no fallback, exactly as before DUT-78.
        let defaults = Self.isolatedDefaults()
        defaults.set(false, forKey: RecipeStore.cloudKitSyncOptInKey)
    ...
    @Test func noCachedStatusPreservesLegacyAttemptCloudKitBehavior() throws {
        // `accountStatus: nil` (never probed — e.g. the very first launch)
        // must NOT force a fallback; it defers entirely to `cloudKitAvailable`
        // (DUT-630's existing Simulator gate), matching pre-DUT-78 behavior.
        let defaults = Self.isolatedDefaults()
        defaults.set(true, forKey: RecipeStore.cloudKitSyncOptInKey)
        let result = try RecipeStore.productionContainer(
            defaults: defaults,
    ...
    @Test func taintedStoreFlagIsNeverSetByAnInMemoryBuild() throws {
        // The taint flag records that a REAL on-disk store was stamped for
        // CloudKit mirroring; an in-memory build never opens `.private(...)`
        // (the DUT-78 in-memory-forces-`.none` guard in
        // `syncedSavedConfiguration`), so it must never be marked tainted —
        // even on the "would otherwise attempt CloudKit" branch (opt-in +
        // available account). Actually marking the flag on a real device open
        // is device-verified, not unit-tested (same posture as the `.private`
        // open itself).
        let defaults = Self.isolatedDefaults()
        defaults.set(true, forKey: RecipeStore.cloudKitSyncOptInKey)
        #expect(RecipeStore.cloudKitStoreEverOpened(in: defaults) == false)
        _ = try RecipeStore.productionContainer(
            defaults: defaults,
            accountStatus: .available,
            launchHealth: nil,
            inMemory: true
