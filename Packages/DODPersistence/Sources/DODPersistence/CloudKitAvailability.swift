...
/// `buildCloudKitWithFallback` do/catch never sees it (that net only wraps
/// the synchronous open). DUT-630 already closed the Simulator instance of
/// this trap (`RecipeStore.cloudKitMirroringAvailable` gates mirroring off
/// there entirely); this type closes the remaining REAL-DEVICE instance,
/// where the account itself — not the runtime — is the problem (signed out
/// of iCloud, a Production schema not provisioned for that account, an
/// iCloud outage). The cheapest way to dodge the async trap is to **not
/// open `.private(...)` in the first place** when we already know the
/// account can't back it: probe `CKContainer.accountStatus()` *before*
/// choosing the configuration, and fall back to a plain local container
/// (marking ``RecipeStore/ContainerBuildResult`` `usedCloudKitFallback`)
...
public enum CloudKitAvailability: Sendable {
    ...
    /// yields `false`, so the app dodges the async mirroring trap and
    /// degrades to "saved recipes stay on this device" until the account
    /// resolves on a later launch (DUT-78 / AC-41.1).
