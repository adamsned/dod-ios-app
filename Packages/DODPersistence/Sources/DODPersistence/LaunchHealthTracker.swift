...
/// `SyncedSaved` store with `.private(...)`. On a device where the CloudKit
/// container can't initialize for the signed-in account (not signed into
/// iCloud, Production schema not provisioned for that account, an outage),
/// Core Data's CloudKit mirroring setup traps with `EXC_BREAKPOINT` on
/// `com.apple.coredata.cloudkit.queue` — **asynchronously, after** the
/// synchronous `ModelContainer(...)` open already returned. The existing
/// `buildCloudKitWithFallback` only wraps the *synchronous* open in
/// do/catch, so it never catches that async trap; the app crash-loops on
/// every relaunch and (because the on-disk store stays tainted once Core
/// Data has stamped it for mirroring) even flipping the opt-in flag back off
/// doesn't recover it — only a reinstall did.
///
/// **What this type does.** It is the last-resort safety net the
/// `CKContainer.accountStatus` pre-flight guard (``CloudKitAvailability``)
/// can't cover — a brand-new failure mode that still traps before that guard
/// would have caught it (e.g. the cached status is stale/missing, or the
/// account itself is `.available` but the Production schema is still wrong
/// for it). At the *very start* of every launch the host records "a launch
/// started but hasn't been marked healthy yet" by incrementing a persisted
/// counter; once the app reaches a known-good point (the SwiftUI scene is on
/// screen and the first run loop turned over) it calls
/// ``markLaunchHealthy()`` to reset the counter to zero. A launch that
/// crashes before `markLaunchHealthy()` therefore leaves the counter
/// incremented. After ``unhealthyLaunchLimit`` consecutive unhealthy
/// launches, ``shouldSelfHeal`` returns `true` and the host force-disables
/// the CloudKit opt-in flag and opens a plain local container — so the app
/// becomes launchable again and the user can reach Settings, instead of
/// being stuck reinstalling.
///
...
/// plugin (DUT-78). The host owns the *timing* of the three calls
