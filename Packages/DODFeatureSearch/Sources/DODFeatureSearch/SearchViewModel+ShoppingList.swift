...
// DUT-534 Part 2 — "Add to Shopping List" from a Search card's long-press menu.
// Split into its own extension file to keep `SearchViewModel.swift` under the
// SwiftLint 400-line `file_length` cap (mirrors Recipe Detail's + Feed's
// `+ShoppingList` splits).

extension SearchViewModel {

    /// Append a card's recipe ingredients to the Shopping List, then surface a
    /// confirming snackbar. A search hit carries a lightweight `RecipeListItem`
    /// with no ingredients, so we hand the shared appender a minimal `Recipe`
    /// built from it (`Recipe(listItem:)`); the appender's hydrate-if-needed step
    /// fetches + parses the ingredients the same way Recipe Detail / the Saved
    /// picker do.
    ///
