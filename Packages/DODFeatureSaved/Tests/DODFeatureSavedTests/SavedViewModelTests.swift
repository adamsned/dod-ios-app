...
@Suite("SavedViewModel (T-130..T-136)") struct SavedViewModelTests {
    ...
    // `FakeSavedDependencies` (used above and by the other Saved-tab test
    // files) lives in `FakeSavedDependencies.swift` — see that file for why.
    // The refresh-vs-refresh race regression test (DUT — stale refresh must
    // not clobber a newer one) lives in `SavedViewModelRefreshRaceTests.swift`
    // to keep this file under SwiftLint's `file_length` cap — same split
    // pattern as `SavedViewModelDownloadWarningTests.swift`.
