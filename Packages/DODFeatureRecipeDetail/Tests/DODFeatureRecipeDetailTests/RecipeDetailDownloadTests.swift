...
@Suite("RecipeDetailViewModel download (T-620)") struct RecipeDetailDownloadTests {
    ...
    @Test func toggleDownloadOfflineRemovesImmediately() async throws {
        // DUT-229 — tapping the filled download button while OFFLINE removes
        // immediately, exactly like online. `removeDownload` only clears the pin
        // (the saved recipe keeps its cached text + pinned hero and still opens
        // offline), so nothing is stranded and the old false confirmation is
        // gone.
        let dependencies = FakeRecipeDetailDependencies()
        ...
        #expect(viewModel.isDownloaded == false)
        #expect(viewModel.snackbarMessage == "Download removed")
        #expect(dependencies.downloadedIDs.contains(626) == false)
        // The recipe stays saved — un-download is not unsave.
        #expect(viewModel.isSaved == true)
    }

    @Test func toggleDownloadOnlineRemovesImmediately() async throws {
        // DUT-229 — the online path is unchanged: removal is instant.
        let dependencies = FakeRecipeDetailDependencies()
