...
public struct FeedView: View {
    ...
    private var list: some View {
        ...
        return ScrollView {
            // The First Cookout nudge used to render HERE as an inline hero card
            // (DUT-571), which pushed the whole feed down for every un-graduated
            // cook. It's now a slim overlay callout hosted by the App shell (see
            // `RootView+FirstCookoutCallout.swift`) so it displaces nothing and can
            // point at the Tools tab, which this view doesn't own.
            Group {
                switch layout {
                case .gallery:
                    galleryContent
